...
import { RotateCcw, CheckSquare, Square, X } from 'lucide-react';
...
const WordQuizFilter = ({
  ...
  return (
    <>
      ...
      {/* Dropdown panel — positioned relative to the header filter button */}
      <div className="absolute top-[72px] right-4 md:top-[88px] md:right-8 z-30 bg-white border border-slate-200 shadow-2xl rounded-2xl p-4 w-72 animate-fade-in text-left">
        <div className="flex items-center justify-between mb-4">
          <p className="text-xs font-black text-slate-700 uppercase tracking-widest">Filter Options</p>
          <button
            onClick={onClose}
        ...
        <div className="space-y-4">
          ...
          {/* Quiz Configuration — hidden in study mode */}
          {!isStudyMode && (
            <>
          ...
          <div>
            ...
            {categories && categories.length > 0 ? (
                    ...
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-bold transition-all group ${
                        isSelected
                          ...
                          : 'hover:bg-slate-50 text-slate-500 bg-white border border-transparent'
                      }`}
                    >
              ...
              <p className="text-xs text-slate-400 text-center py-4">No categories available</p>
            )}
          </div>
        </div>
      </div>
