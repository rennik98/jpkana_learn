...
const SentenceQuizPage = ({ lessonSentences, onComplete, isLessonMode = false }) => {
  ...
  const dataSource = (isLessonMode && lessonSentences && lessonSentences.length > 0)
    ? lessonSentences
    : SENTENCE_DATA;
  ...
  const generateQuestion = useCallback(() => {
    ...
    const item = dataSource[randomIndex];
  ...
  const handleNext = () => {
    ...
    if (isLessonMode && onComplete && nextCount >= dataSource.length) {
      onComplete();
      return;
    }
    generateQuestion();
        ...
        <div className="flex items-center gap-4 sm:gap-6">
          ...
          {isLessonMode && (
            <div className="flex flex-col">
              ...
              <span className="text-xl font-bold text-blue-600 leading-none">{answeredCount}/{dataSource.length}</span>
            </div>
          )}
        </div>
          ...
          <p className="text-3xl sm:text-4xl font-bold text-slate-800 mb-6 leading-relaxed">
            {currentQuestion.sentence.split('___').map((part, i, arr) => (
              <React.Fragment key={i}>
                {part}
                {i < arr.length - 1 && (
                  <span className={`inline-block min-w-[120px] border-b-4 mx-2 transition-all duration-300 ${
                    feedback
                      ? (feedback === 'correct' ? 'text-green-600 border-green-200' : 'text-red-500 border-red-200')
                      : 'border-slate-300'
                  }`}>
          ...
          <div className={`transition-all duration-300 overflow-hidden ${feedback ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'}`}>
            <div className={`p-4 rounded-2xl flex flex-col gap-2 ${
              ...
              <div className="flex items-center justify-between">
                ...
                >
                  {isLessonMode && answeredCount + 1 >= dataSource.length ? 'Finish' : 'Next'} <ArrowRight size={14} />
                </button>
              </div>

              <div className={`text-xs p-2 rounded-lg border mt-1 ${
                feedback === 'correct' ? 'bg-white/60 border-green-200' : 'bg-white/60 border-red-200'
              }`}>
