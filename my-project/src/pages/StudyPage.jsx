  ...
  return (
    <div className="h-full flex flex-col animate-fade-in relative">
      ...
      <div onClick={() => setShowSettings(false)}>
        {studyMode === 'kana' ? (
          // Grouped by section for kana mode
          [
            { key: 'basic',   label: 'พื้นฐาน — Basic' },
            { key: 'dakuten', label: 'เสียงขุ่น — Voiced' },
            { key: 'youon',   label: 'เสียงควบ — Compound' },
          ].map(({ key, label }) => {
            const items = dataList.filter(item => item.section === key);
                    ...
                    return (
                      <div key={idx} className="group bg-white rounded-2xl p-4 shadow-sm border border-slate-100 hover:shadow-md hover:border-slate-200 transition-all duration-200 flex flex-col items-center relative">
                        <div className="flex-1 flex items-center justify-center min-h-[5rem]">
                          <span className="font-bold text-slate-800 text-5xl">{content.main}</span>
                        </div>
                        <div className="mb-4">
          ...
          // Flat grid for numbers and words
