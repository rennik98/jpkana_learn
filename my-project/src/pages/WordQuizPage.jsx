...
const WordQuizPage = ({
  ...
  const [uniqueMode, setUniqueMode] = useState(isLessonMode);
  ...
  }, [filteredWords, mode, uniqueMode, masteredIndices, isLessonMode, onComplete]);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto h-full flex flex-col pb-4 animate-fade-in relative">
          ...
          {uniqueMode && (
            <div className="flex flex-col">
              ...
              <span className="text-xl font-bold text-blue-600 leading-none">
                {masteredIndices.length}/{filteredWords.length}
              </span>
            </div>
          )}
        </div>
      ...
      {!isLessonMode && (
        <WordQuizFilter
          isOpen={showSettings}
          onClose={() => setShowSettings && setShowSettings(false)}
      ...
      {currentQuestion && (
        <div
          ...
          onClick={() => setShowSettings && setShowSettings(false)}
        >
          ...
          <div className="p-4 sm:p-6 bg-white shrink-0 flex flex-col gap-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {currentQuestion.options.map((option, idx) => {
                const answerText = mode === 'jp-en'
                  ? option.en
                  : (difficulty === 'easy' ? `${option.jp} (${option.romaji})` : option.jp);
